...
import {
  ArrowLeft, Loader2, FileDown, Send, Copy, Trash2, ExternalLink,
  CheckCircle, Edit, FileText, Eye,
} from 'lucide-react';
...
export default function CrmQuoteDetailPage() {
  ...
  const handleUpdateQuote = async (data: QuoteCreate) => {
    ...
    setError(null);
    try {
      await api.updateQuote(quoteId, data);
      await fetchQuote();
      setIsEditing(false);
  ...
  const handleSendEmail = async () => {
    if (!quote?.customer?.email) { setError('Customer has no email address'); return; }
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      router.push(`/admin/crm/quotes/${newQuote.id}`);
    } catch {
  ...
  const formatCurrency = (amount: number | string) =>
    new Intl.NumberFormat('en-US', { style: 'currency', currency: quote?.currency || 'USD' }).format(Number(amount));
  ...
  if (!quote) {
    return (
      <CrmLayout title="Quote Not Found">
        <div className="text-center py-20">
          <h2 className="text-xl font-semibold mb-2">Quote not found</h2>
          <Link href="/admin/crm/quotes" className="text-blue-600 hover:underline">Back to Quotes</Link>
        </div>
      </CrmLayout>
    );
  }
  ...
  return (
    <CrmLayout title={`Quote ${quote.reference}`}>
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <Link href="/admin/crm/quotes" className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {quote.pdf_url ? (
              ...
              <button onClick={handleGeneratePdf} disabled={actionLoading === 'pdf'} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
                <FileDown className="w-4 h-4" /> {actionLoading === 'pdf' ? 'Generating...' : 'Generate PDF'}
              </button>
            )}
            <button onClick={handleSendEmail} disabled={actionLoading === 'email' || !quote.customer?.email} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
              <Send className="w-4 h-4" /> {actionLoading === 'email' ? 'Sending...' : 'Send Email'}
            </button>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {quote.status === 'draft' && (
                  <button onClick={() => handleStatusChange('sent')} disabled={actionLoading === 'status'} className="text-sm px-3 py-1 bg-blue-100 text-blue-700 rounded-full hover:bg-blue-200">Mark as Sent</button>
                )}
                {quote.status === 'sent' && (
                  <button onClick={() => handleStatusChange('accepted')} disabled={actionLoading === 'status'} className="text-sm px-3 py-1 bg-green-100 text-green-700 rounded-full hover:bg-green-200">Mark as Accepted</button>
                )}
                {quote.status === 'accepted' && (
                  quote.invoice_id ? (
                    <Link href={`/admin/crm/invoices/${quote.invoice_id}`} className="text-sm px-3 py-1 bg-green-100 text-green-700 rounded-full hover:bg-green-200">
                      <FileText className="w-3 h-3 inline mr-1" /> View Invoice
                    </Link>
                  ) : (
                    <button onClick={handleConvertToInvoice} disabled={actionLoading === 'invoice'} className="text-sm px-3 py-1 bg-purple-100 text-purple-700 rounded-full hover:bg-purple-200">
                      <FileText className="w-3 h-3 inline mr-1" /> {actionLoading === 'invoice' ? 'Creating...' : 'Create Invoice'}
                    </button>
                  )
                )}
                <button onClick={handleDuplicate} disabled={actionLoading === 'duplicate'} className="text-sm px-3 py-1 bg-gray-100 text-gray-700 rounded-full hover:bg-gray-200">
            ...
            <div className="mt-6 pt-6 border-t">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 bg-gray-100 px-3 py-2 rounded text-sm">
                  {typeof window !== 'undefined' ? `${window.location.origin}${quote.public_url}` : quote.public_url}
                </code>
                <button
                  onClick={() => { navigator.clipboard.writeText(`${window.location.origin}${quote.public_url}`); showSuccess('Link copied'); }}
                  className="px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded"
                >Copy</button>
                <Link href={quote.public_url} target="_blank" className="px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded">
              ...
              <tbody className="divide-y">
                {quote.line_items.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="py-4 text-center">{item.is_service ? '—' : item.quantity}</td>
                    <td className="py-4 text-right">{formatCurrency(item.unit_price)}</td>
                    <td className="py-4 text-right font-medium">{formatCurrency(item.line_total || 0)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="border-t mt-4 pt-4">
            <div className="flex justify-end">
              <div className="w-64 space-y-2">
                ...
                {quote.tax_amount > 0 && (
                  <div className="flex justify-between"><span className="text-gray-500">Tax ({quote.tax_rate}%)</span><span>{formatCurrency(quote.tax_amount)}</span></div>
                )}
                <div className="flex justify-between text-lg font-bold border-t pt-2"><span>Total</span><span>{formatCurrency(quote.total)}</span></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border-2 border-red-200">
          ...
          <p className="text-gray-600 mb-4">Once you delete a quote, there is no going back.</p>
          <button onClick={handleDelete} disabled={actionLoading === 'delete'} className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50">
            <Trash2 className="w-4 h-4" /> {actionLoading === 'delete' ? 'Deleting...' : 'Delete Quote'}
          </button>
        </div>
      </div>
    </CrmLayout>
