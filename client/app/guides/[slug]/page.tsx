...
async function getPost(slug: string) {
  try {
    ...
    if (post.content_type !== 'guide') return null;
    return post;
  } catch {
    return null;
  }
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!post) {
    return { title: 'Guide Not Found | Tola Tiles' };
  }

  return {
    title: `${post.effective_meta_title} | Tola Tiles Guides`,
    description: post.effective_meta_description,
    alternates: {
      canonical: post.canonical_url || `https://tolatiles.com/guides/${post.slug}`,
    },
