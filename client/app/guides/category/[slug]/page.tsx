...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!category) return { title: 'Category Not Found | Tola Tiles Guides' };

  return {
    title: `${category.name} - Tile Guides Jacksonville & St. Augustine FL | Tola Tiles`,
...
export default async function GuideCategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <ContentCategoryPage category={category} posts={posts} contentType="guide" location="florida" />;
}
