...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!post) {
    return { title: 'Design Idea Not Found | Tola Tiles' };
  }

  return {
    title: `${post.effective_meta_title} | Tola Tiles Design Ideas`,
...
export default async function DesignIdea({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <ContentDetailPage post={post} relatedPosts={relatedPosts} contentType="design_idea" location="florida" />;
}
